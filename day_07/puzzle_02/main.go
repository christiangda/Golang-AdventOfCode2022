...
func init() {
	...
	flag.IntVar(&fsSize, "maxSize", 70000000, "The total disk space available to the filesystem")
	flag.IntVar(&fsUpdateSize, "fsUpdateSize", 30000000, "The size needed by the update")
...
func main() {
	...
	rootSize := root.Size
	sizeAvailable := fsSize - rootSize

	sizeToFree := fsUpdateSize - sizeAvailable

	gotSize := root.GetGreaterOrEqual(sizeToFree)

	fmt.Println("-----------------------------------------------------------------")
	fmt.Printf("The total disk space available to the filesystem: %v\n", fsSize)
	fmt.Printf("Used in / (root): %v\n", rootSize)
	fmt.Printf("Size Available: %v\n", sizeAvailable)
	fmt.Printf("Update needed space size: %v\n", fsUpdateSize)
	fmt.Printf("\nNeeded space to be free (at least): %v\n", sizeToFree)

	fmt.Printf("\nWhat is the Node in the tree selected?: %v\n", gotSize.Name)
	fmt.Printf("What is the total size of that directory?: %v\n", gotSize.Size)
}
